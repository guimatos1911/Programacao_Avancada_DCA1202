...
void WhatsProgDadosServidor::enviarMensagem(iterMensagem iMsg, iterUsuario iDest)
{
  try
  {
    ...
    iterUsuario remetente = find(LU.begin(), LU.end(), iMsg->getRemetente());
    if (remetente == LU.end()) throw 6; // Remetente não encontrado

    // Remetente existe. Testa se está conectado.
    if (remetente->connected())
    {
      // Remetente está conectado. Envia a confirmação de entrega da mensagem.
  ...
  catch (int erro)
  {
    if (erro >= 2 && erro <= 5)
    {
      // Desconecta o destinatário se houve erro no envio pelo socket
      iDest->close();
    }
    cerr << "Erro " << erro << " no envio da mensagem para destinatário "
         << iDest->getLogin() << endl;
  }
}
          ...
          for (iU=LU.begin(); !fim && iU!=LU.end(); ++iU)
          {
            if (!fim && iU->connected() && f.had_activity(iU->getSocket()))
            {
              ...
              case CMD_NOVA_MSG:
                ...
                iDest = find(LU.begin(), LU.end(), destinatario);
