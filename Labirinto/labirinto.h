...
///Classe noh
class Noh{
  public: //necessario ser public para que o algoritmo A* possa ler
    Coord pos;
    Coord ant;
    double g;
    double h;
    ...
    //construtores
    Noh(): pos(Coord(-1, -1)), ant(Coord(-1, -1)), g(-1), h(-1) {}
    Noh(Coord& N, Coord& C, double g, double h) : pos(N), ant(C), g(g), h(h) {}

    //funcoes e operadores
    double cTotal() const {return g + h;}
    bool operator==(const Coord& P) const {return P == pos;}
    bool operator()(const Noh& N) const{return cTotal() < N.cTotal();}
};
